                      ...
                      <div className="w-12 h-12 rounded-xl bg-slate-100 overflow-hidden flex items-center justify-center">
                        {p.productImg
                          ? <img 
                            ...
                            />
                          : null}
                        <div style={p.productImg ? { display: 'none' } : {}} className="w-full h-full flex items-center justify-center">
                          <Package className="w-5 h-5 text-slate-300" />
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3 font-semibold text-slate-800 max-w-[180px] truncate">{p.productTitle}</td>
                    <td className="px-5 py-3 font-bold text-slate-800">${(p.productPrice || 0).toFixed(2)}</td>
                    <td className="px-5 py-3 text-slate-500 line-through text-xs">${(p.productPspPrice || 0).toFixed(2)}</td>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl p-8 my-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input type="number" step="0.01" min="0" value={modal.data.productPrice} onChange={e => setModal({ ...modal, data: { ...modal.data, productPrice: e.target.value } })}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand" placeholder="0.00" />
              </div>
              <div>
                <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1.5 block">PSP Price ($)</label>
                <input type="number" step="0.01" min="0" value={modal.data.productPspPrice} onChange={e => setModal({ ...modal, data: { ...modal.data, productPspPrice: e.target.value } })}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-brand/20 focus:border-brand" placeholder="0.00" />
              </div>
              <div>
