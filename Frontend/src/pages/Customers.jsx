...
const Customers = () => {
  ...
  const handleSave = async () => {
    setSaving(true);
    try {
      await customerApi.update(editModal.customerId, editModal);
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map((c, i) => (
                  <tr key={c.customerId} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-5 py-4">
                      <button onClick={() => setEditModal({ ...c })} className="p-2 rounded-lg text-slate-400 hover:text-brand hover:bg-brand/10 transition-all">
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg p-8">
            ...
            <div className="space-y-4">
              {[
                { key: 'customerName', label: 'Full Name', icon: User },
                { key: 'customerEmail', label: 'Email', icon: Mail },
                { key: 'customerContact', label: 'Contact', icon: Phone },
                { key: 'customerCity', label: 'City', icon: MapPin },
                { key: 'customerCountry', label: 'Country', icon: MapPin },
                { key: 'customerAddress', label: 'Address', icon: MapPin },
            ...
            <div className="flex space-x-3 mt-8">
              <button onClick={() => setEditModal(null)} className="flex-1 py-3 border border-slate-200 rounded-xl text-slate-600 font-medium hover:bg-slate-50 transition-all">Cancel</button>
